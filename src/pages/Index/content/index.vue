...
import getStaytime from "@/pages/Index/content/hook/getstaytime/getStaytime";

//获取停留时间
const getstaytime = async () =>{
  const result = await getStaytime(query.UUID as string)
  stayTimeInfo.xAxis.data = result.stayX
  stayTimeInfo.series[0].data =result.stayY
  stayTime.setOption(stayTimeInfo)
}
</script>
    ...
    <div class="flex justify-between w-screen mt-20 font-sans">
        <div class="flex-1">
            <div class="flex flex-col justify-center ml-20 mt-1">
                ...
                <div class="w-10/12 h-40 shadow-2xl mt-3 rounded-xl border" style="height: 200px" id="stayTime"></div>
            </div>
        </div>
        <div class="flex-1 justify-center items-center transition duration-150 ease-in-out hover:scale-105">
            <div class="w-11/12 border mt-10 shadow-xl rounded-xl" style="height: 500px" id="countInfo"></div>
        </div>
    </div>
</template>
<style scoped></style>
